...
intents.matches(/^joke/i, [
    function (session) { 
        builder.Prompts.text(session, 'categories: other');
       
    },
    function (session) {
        session.beginDialog('/jokes');
    }
]);
...
intents.onDefault([
    ...
    function (session, results) {
        ...
        session.send("%s, enter 'joke' ;) ", session.userData.name);
    }
]);

bot.dialog('/jokes', [
    function(session){
        session.send('Just because you can’t dance doesn’t mean you shouldn’t dance. —Alcohol');
        session.endDialog();
    }
]);
